...
def calc_next_joint_position(cur_position: float, target_position: float, loops_left: int):
...
def calc_next_positions(joint_positions: List[float],
                        target_point: JointTrajectoryPoint,
                        goal_start_time: rospy.Time, loop_hz, now_time: rospy.Time):

    time_left = (goal_start_time + target_point.time_from_start) - now_time
    if time_left < rospy.Duration(0):
        time_left = rospy.Duration(0)

    loops_left = ceil(time_left.to_sec() / (1 / loop_hz))

    new_positions = [0] * len(joint_positions)

    # For each joint
    for joint_idx in range(len(joint_positions)):
        new_positions[joint_idx] = calc_next_joint_position(
            joint_positions[joint_idx], target_point.positions[joint_idx], loops_left
        )
        print(f"[{now_time}] Joint {joint_idx} is at {joint_positions[joint_idx]} and needs to be at {target_point.positions[joint_idx]} so it will move next to {new_positions[joint_idx]}")
...
class R2B2ArmController:
    ...
    def execute(self, goal: FollowJointTrajectoryGoal):
        ...
        while self.action_server.is_active():
            ...
            if is_at_waypoint(self.joint_positions, cur_point, self.path_tolerance):
            ...
            # Have we exceeded our goal time?
            ...
            next_positions = calc_next_positions(
                self.joint_positions, cur_point, start_time, self.loop_hz, rospy.Time.now()
            )
